...
const profileUpdateSchema = z.object({
  ...
  highlightStyle: z.enum(['background', 'underline']).optional(),
});
...
export async function updateUserProfile(formData: FormData): Promise<{ success: boolean; message?: string }> {
  ...
  const result = profileUpdateSchema.safeParse({
    ...
    highlightColor: formData.get('highlightColor') || undefined,
    highlightStyle: formData.get('highlightStyle') || undefined,
  });
  ...
  try {
    ...
    const updatedUser: User = {
        ...
        highlightColor: result.data.highlightColor ?? user.highlightColor,
        highlightStyle: result.data.highlightStyle ?? user.highlightStyle,
    };
